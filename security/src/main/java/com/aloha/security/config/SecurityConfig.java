...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(
            auth -> auth.requestMatchers("/admin","/admin/**").hasRole("ADMIN")
		...
		// ✅ 커스텀 로그인 페이지
        http.formLogin( 
            login -> login.loginPage("/login")						       // 커스텀 로그인 페이지 경로
                          .loginProcessingUrl("/login")		       // 로그인 요청 경로
                       // .usernameParameter("username")            // 아이디 파라미터 명
        ...
        http.exceptionHandling(
            exception -> exception.accessDeniedHandler(customAccessDeniedHandler)   // 접근 거부 처리자 설정
                               // .accessDeniedPage(null)    // 예외 처리 페이지 설정
        );
    ...
    */
    ...
    //                 ;
    //     // 사용자 권한 쿼리
    //     String sql2 = " SELECT username, auth AS authority "
    //                 + " FROM user_auth "
    //                 + " WHERE username = ? "
    //                 ;
