...
public class BoardController {
	...
	public ResponseEntity<?> create(@AuthenticationPrincipal CustomUser customUser, @RequestBody Board board) {
		boolean result;
		try {
			// 현재 인증된 사용자 번호(no) 를 등록 요청한 게시글 데이터에 저장
	 ...
	 * @throws Exception
	 * @PreAuthorized
	 * - 파라미터 값을 가져오는 방법 : #p0, #p1 형태로 파라미터 인덱스를 지정하여 가져올 수 있다.
	 * * 여기서는 요청 파라미터로 넘어온 id 1번째에 있기 때문에
	 * 	 인덱스로는 1번이 되서 => #p1
	...
	public ResponseEntity<?> delete(@PathVariable("id") String id) {
		boolean result;
		try {
			result = boardService.deleteById(id);
			if ( !result ) {
				return new ResponseEntity<>("FAIL", HttpStatus.BAD_REQUEST);
			}
			return new ResponseEntity<>("SUCCESS", HttpStatus.OK);
