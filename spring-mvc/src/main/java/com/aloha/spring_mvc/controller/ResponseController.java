...
public class ResponseController {
	...
	public ModelAndView responseModelAndView(ModelAndView mv) {
		...
		log.info("response/model/view.html 뷰를 응답");
		// 뷰 지정
	 ...
	 * - 응답	   : board (XML/JSON)
	 * @ResponseBaby : 응답하는 데이터를 본문(body) 에 지정하는 어노테이션
	...
	public ResponseEntity<String> ResponseEntityString() {
		// return new ResponseEntity<>(응답 본문, 상태코드);
	...
	/**
	 * ResponseEntity<?>
	 * - 타입 매개변수를 ? 와일드카드로 지정하면 
	 *   응답 타입을 유연하게 사용할 수 있다.
	 * ResponseEntity 를 사용하면
	 * @ResponseBody 를 생략해도 된다.
	 * : 뷰 해석 단계를 건너뛰고 응답 본문에 직접 처리해준다. 
	 * @return
	 * @throws Exception
	 */
	...
	public ResponseEntity<byte[]> responseFile(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		// return new ResponseEntity<byte[]>(응답 본문, 헤더, 상태코드);
