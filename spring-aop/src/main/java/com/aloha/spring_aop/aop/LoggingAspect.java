...
public class LoggingAspect {
	...
	// Advice
	// Point Cut : execution( 접근 제한자 반환 타입 패키지.클래스.메서드(파라미터) )
	// Join Pint : @Before, @After, @Around 등
	...
	public void before(JoinPoint jp) {
		...
		log.info("signature : " + jp.getSignature().toString());
	...
	*/
	...
	// pointcut		: 포인트컷 표현식
	// returning	: 타겟 메소드의 반환값을 저장할 매겨변수명 지정
	@AfterReturning(
	...
	public void printParam(JoinPoint jp) {
		...
		if( parameterNames != null ) {
			for (int i = 0; i < parameterNames.length; i ++) {
				...
				log.info("파라미터 명 : {}, 값 : {}", paramName, paramValue);
			}
		}
	}
